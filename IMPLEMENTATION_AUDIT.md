...
## 11. Current Implementation vs Paper (as of audit update)

**Now aligned in code:** BCSS + Lasso feature selection, jump-penalty scaling by 1/√n_features, Nystrup-style online inference (lookback window, fixed centroids, last state), T+2 regime application, optional daily BL application. So the *structure* is closer; remaining gaps are data (SPY vs MSCI), exact tuning procedure, and VIX feature sequence.

---

## 12. Explaining OOS Drift (Helix vs SPY / EW7)

If the implementation is broadly aligned, the **slow widening underperformance** of Helix vs SPY and EW7 after the OOS start (despite tracking closely in-sample) can be explained by:

1. **Regime / expected-return error** — Regimes are fitted on pre-OOS history. OOS, the true regime process may differ (different volatility, factor behavior, or correlation structure). Expected returns from "historical average in inferred regime" then mis-specify, so BL tilts in the wrong direction and the gap widens over time.

2. **Overfitting** — SJM hyperparameters (λ, κ) and regime centroids are chosen or fitted on in-sample data. Out-of-sample, regime boundaries and feature relevance can shift. The model continues to assign regimes and expected returns that were optimal in-sample but are wrong OOS, so active tilts add noise and transaction cost rather than alpha.

3. **Data and universe** — Paper uses Bloomberg and MSCI USA (PBUS); we use yfinance and SPY. Different indices and data vintages imply different factor premia and correlations. What worked in the paper universe may not translate to our ETF universe in the same period.

4. **Cumulative effect of small tilts** — Daily BL weights are small deviations from the benchmark. Small but persistent estimation error in expected returns compounds; the strategy tracks SPY/EW7 at first, then drifts as the cumulative effect of wrong tilts and rebalancing costs shows up.

5. **Weak or zero factor-timing alpha in the OOS window** — The paper results may be period- or universe-specific. In our OOS window (e.g. 2024–2025), factor-timing alpha may be weak or zero. Then any active tilt is mostly noise; net of costs and tracking error, Helix underperforms a simple SPY or EW7 hold.

6. **Regime stickiness vs reality** — If regimes are too sticky, the strategy under-reacts to regime changes OOS; if too jumpy, it over-trades on noise. In both cases, the result can be underperformance relative to a passive benchmark.

**Bottom line:** Close tracking in-sample with widening underperformance OOS is consistent with (a) implementation roughly aligned but (b) regime/expected-return estimates that do not generalize, and/or (c) little or no factor-timing alpha in our data and period. It does not by itself imply a single bug; it suggests validation (e.g. single-factor long-short Sharpe, stability of λ/κ across windows) and possibly different data or a longer OOS evaluation.
